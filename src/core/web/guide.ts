...
<li><b>Agent chỉ làm trong folder đó.</b> Mỗi lượt chạy được dặn rõ folder làm việc. Với Claude, ekip gắn thêm một hàng rào: mọi lần đọc, ghi, tìm file hay chạy lệnh chạm tới folder khác đều bị chặn và báo lại cho agent, transcript hiện dòng cảnh báo. Với Gemini (Antigravity không có hook), trên macOS ekip chạy agy trong <b>sandbox của hệ điều hành</b>: đọc hay ghi vào folder khác trong thư mục home, hoặc vào <code>~/.ssh</code>, <code>~/.aws</code>, đều bị hệ điều hành từ chối. Trên Linux và Windows, Gemini chỉ được dặn.</li>
...
<li><b>Agents stay inside that folder.</b> Every run is told its working folder. For Claude, ekip adds a guard: any read, write, search or command that reaches another folder is blocked and explained to the agent, and the transcript shows a warning. For Gemini (Antigravity has no hook), on macOS ekip runs agy inside the <b>operating system's sandbox</b>: reading or writing another folder in your home, or <code>~/.ssh</code> and <code>~/.aws</code>, is refused by the OS. On Linux and Windows, Gemini runs get the instruction only.</li>
