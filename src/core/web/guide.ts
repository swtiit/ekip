...
<li><b>Agent chỉ làm trong folder đó.</b> Mỗi lượt chạy được dặn rõ folder làm việc. Với Claude, ekip gắn thêm một hàng rào: mọi lần đọc, ghi, tìm file hay chạy lệnh chạm tới folder khác đều bị chặn và báo lại cho agent, transcript hiện dòng cảnh báo. Với Gemini (Antigravity không có hook), trên macOS ekip chạy agy trong <b>sandbox của hệ điều hành</b>: chỉ ghi được vào folder và chỗ agent giữ cache; đọc folder khác trong home hay các file credential (<code>~/.ssh</code>, <code>~/.aws</code>, <code>~/.npmrc</code>, profile trình duyệt…) đều bị hệ điều hành từ chối. Trên Linux và Windows, Gemini chỉ được dặn.</li>
<li><b>Agent không điều khiển được hub như bạn.</b> Lượt chạy nhận một token riêng chỉ mở được kênh MCP, nên nó dùng được công cụ bridge trong phạm vi task của nó, nhưng không gọi được API để mở việc ở folder khác. Token của bạn (xem bằng <code>ekip token</code>) mới mở được cả API.</li>
<li><b>Mỗi folder chỉ một agent sửa file tại một thời điểm.</b> Agent có quyền sửa (Lập trình · Claude, Lập trình · Gemini) mà gặp một agent khác đang sửa cùng folder thì xếp hàng chờ, nên không ghi đè lên nhau. Agent chỉ đọc như Review hay Điều phối không bị giữ lại.</li>
...
<li><b>Agents stay inside that folder.</b> Every run is told its working folder. For Claude, ekip adds a guard: any read, write, search or command that reaches another folder is blocked and explained to the agent, and the transcript shows a warning. For Gemini (Antigravity has no hook), on macOS ekip runs agy inside the <b>operating system's sandbox</b>: it can write only to the folder and its own caches, and reading other home folders or credentials (<code>~/.ssh</code>, <code>~/.aws</code>, <code>~/.npmrc</code>, browser profiles…) is refused by the OS. On Linux and Windows, Gemini runs get the instruction only.</li>
<li><b>An agent can't drive the hub the way you can.</b> A run gets its own token, good for the MCP channel only: it can use the bridge tools within its task, but cannot call the API to start work in another folder. Yours (see <code>ekip token</code>) opens both.</li>
<li><b>One agent edits a folder at a time.</b> A member that can edit files (the Claude or Gemini coder) waits in line while another editor works in the same folder, so they can't overwrite each other. Read-only members like the reviewer or conductor are never held up.</li>
